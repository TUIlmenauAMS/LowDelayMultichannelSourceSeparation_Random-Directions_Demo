...
def abskl(Xunm):  
   ...
   abskl+=np.sum( X_abs[:,1] * np.log((X_abs[:,1]+1e-6)/(X_abs[:,0]+1e-6)) ) #for symmetry
   return -abskl
...
def blockseparationoptimization(coeffs, Xblock, state0, state1):
   ...
   for m in range(2):
      ...
      coeffvariation=4*(np.random.rand(4)-0.5)*coeffweights  #small variation
      #coeffvariation=np.random.normal(loc=0.0, scale=0.5, size=4) #Gaussian distribution
      #new values 1:
      Xunm1, state01, state11 =unmixing(coeffs+coeffvariation, Xblock, state0, state1)
...
if __name__ == '__main__':
   ...
   for m in range(Blocks):
      ...
      state1[:,:-1]=state1[:,1:]
      Xunm, coeffs, state0[:,-1], state1[:,-1] = blockseparationoptimization(coeffs, sigmemory, state0[:,-M-1], state1[:,-M-1])
